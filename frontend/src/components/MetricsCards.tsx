  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      {metrics.map((m) => (
        <div
          key={m.label}
          className={`bg-white rounded-2xl border border-gray-100 shadow-card p-5 border-l-4 ${m.accent} flex items-center gap-4`}
        >
          <div className={`${m.iconBg} rounded-xl p-3 flex-shrink-0`}>
            <m.icon className={`h-5 w-5 ${m.iconColor}`} />
          </div>
          <div className="min-w-0">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide truncate">{m.label}</p>
