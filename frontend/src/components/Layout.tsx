...
import {
  ...
  FileStack, ChevronDown, Loader2, GitBranch, LogOut,
  ChevronRight, Menu, X,
} from "lucide-react";
  ...
  if (loading && jobs.length === 0) {
    return (
      <div className="mx-3 px-3 py-2 rounded-xl bg-slate-100 flex items-center gap-2 text-xs text-slate-500">
        ...
        {!collapsed && <span>Loading jobs…</span>}
      </div>
    );
  }
    ...
    <div className="relative mx-3" ref={ref}>
      ...
      {open && (
        <div className="absolute left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-100 z-50 py-1 max-h-64 overflow-y-auto">
          <p className="px-3 py-1.5 text-xs font-semibold text-gray-400 uppercase tracking-wider">Active job</p>
          {jobs.map((job) => (
            <button
              key={job.id}
              type="button"
            ...
            >
              {job.id === activeJob?.id
                ? <div className="h-1.5 w-1.5 rounded-full bg-blue-600 flex-shrink-0 ml-0.5" />
                : <div className="h-1.5 w-1.5 rounded-full bg-gray-200 flex-shrink-0 ml-0.5" />
              }
...
export default function Layout({ children }: LayoutProps) {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className={`flex items-center gap-3 px-4 h-16 border-b border-slate-100 flex-shrink-0 ${collapsed ? "justify-center" : ""}`}>
        ...
        {!collapsed && (
          <span className="text-base font-bold text-slate-900 tracking-tight">FairHire AI</span>
        )}
      </div>
      ...
      <div className="py-3 border-b border-slate-100 flex-shrink-0">
        {!collapsed && (
          <p className="px-4 mb-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">Active Job</p>
        )}
      ...
      <nav className="flex-1 py-4 space-y-0.5 px-2 overflow-y-auto">
        ...
        {NAV_ITEMS.map(({ path, icon: Icon, label, desc }) => {
          ...
          return (
            ...
            >
              <Icon className={`h-4.5 w-4.5 flex-shrink-0 ${active ? "text-white" : "text-slate-400 group-hover:text-slate-600"}`} style={{ height: "1.125rem", width: "1.125rem" }} />
              {!collapsed && (
  ...
  return (
    <div className="min-h-screen flex bg-slate-50">
      {/* Desktop sidebar */}
      <aside
        className={`hidden lg:flex flex-col bg-white border-r border-slate-100 shadow-sm flex-shrink-0 transition-all duration-200 ${
          collapsed ? "w-16" : "w-60"
        }`}
      ...
      >
        ...
        <button
          ...
          className="absolute -right-3 top-20 bg-white border border-slate-200 rounded-full p-1 shadow-sm text-slate-400 hover:text-slate-700 hover:border-slate-300 transition-colors z-10"
